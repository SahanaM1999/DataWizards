...
def donut_chart(age,sex,fvc_all):
  ...
  for i in range(len(fvc_all)):
    risk_type=fetch_risk_type(age,sex,int(fvc_all[i]))
    if risk_type=='Normal':
      nc+=1
      ...
      sc+=1
  stage=''
  a=((nc/52)*100)#normal percent
  b=((mc/52)*100)#mild risk percent
  c=((sc/52)*100)#severe risk percent
  if max(a,b,c)==c and c>50:
      stage='Stage 4'
  elif max(a,b,c)==c and c<50 :
      stage='Stage 3'
  elif max(a,b,c)==b and b>50:
      stage='Stage 2'
  elif max(a,b,c)==b and b<50:
      stage='Stage 1'
  else:
...
def progression():
    ...
    donut,risk_type,stage_new=donut_chart(age_var,sex_var,new_res)

    #os.remove('temp.csv')
    #os.remove(file_path)
    return render_template('index2.html',output=new_out,pat_id=pat_id,age=age[0],min_fvc=min_fvc,avg_fvc=avg_fvc,donut=donut,area=area,risk_type=risk_type,stage_new=stage_new)
